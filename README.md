...
- **No se usan `canvasWidth`/`canvasHeight` de `html-to-image`.** Esa opción pinta el SVG intermedio directamente al tamaño final, y ahí los navegadores difieren: el SVG lleva `viewBox`, así que **Firefox respeta `preserveAspectRatio` y deja bandas blancas**, mientras que Chrome lo estira. En su lugar se rasteriza a resolución nativa y se escala **canvas → canvas**: con un origen de mapa de bits, `drawImage` siempre estira al rectángulo de destino igual en todos los navegadores. De propina, el `pixelRatio` se calcula para rasterizar ya al ancho final, así que el texto sale nítido en vez de ampliado desde el tamaño en pantalla.
- Se le pasan además las medidas fraccionarias reales del nodo —la librería mide con `clientWidth`/`clientHeight`, que son enteros— y se avisa por consola si la proporción se desvía más del 0,5 % de la pedida.
