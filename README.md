...
- Eso importa para la exportación: el SVG que genera `html-to-image` lleva `viewBox`, así que al pintarlo en el canvas se ajusta con `preserveAspectRatio` (`xMidYMid meet`). Cualquier diferencia de proporción entre el nodo y el canvas se convierte en **bandas blancas**, no en un estirado. Por eso se le pasan además las medidas fraccionarias reales del nodo (la librería mide con `clientWidth`/`clientHeight`, que son enteros) y se avisa por consola si la desviación supera el 0,5 %.
